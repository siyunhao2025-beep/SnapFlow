...
for(const name of ['robots.txt','sitemap.xml','README.md','config.js'])if(fs.existsSync(path.join(src,name)))fs.copyFileSync(path.join(src,name),path.join(out,name))
for(const name of ['docs','admin'])if(fs.existsSync(path.join(src,name)))fs.cpSync(path.join(src,name),path.join(out,name),{recursive:true})
